...
**There is a moderate positive correlation between number of runs and number of at bats**
...
```{r}
homerun_model <- lm(runs ~ homeruns, data = mlb11)
summary(homerun_model)
```

$$ \hat{y} = 415.2389 + 1.8345 \cdot homeruns $$

**The least squares line tells us that the number of runs can be expected to increase on average by 1.8345 per homerun, while home runs explain 62.66% of the variability in the model.**
...
p <- ggplot(mlb11, aes(sample = at_bats)) +  # Create QQplot with ggplot2 package
  stat_qq() +
  stat_qq_line(col = "red")

p

#p + transition_reveal(mlb11$at_bats)
  p + 
  transition_layers(1)
```
...
```{r}
atbats <- 5579
estimated_runs_by_atbats <- -2789.2429 + 0.6305*atbats
actual_runs_by_at_bats <- mlb11$runs[which(mlb11$at_bats == 5579)]

residual <- actual_runs_by_at_bats - estimated_runs_by_atbats

estimated_runs_by_atbats
actual_runs_by_at_bats
residual
```
...
**The residuals mostly cluster around the line, indicating a moderate correlation between at bats and number of runs**
...
**The residuals are roughly normal, and the fitted line is linear**
...
**Yes, the points are all around and 0 and the variability remains constant**
