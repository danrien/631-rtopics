...
lower <= true_mean && true_mean <= upper
```

**Yes, the mean is within the confidence interval**
...
```{r}
sum(true_mean < lower | true_mean > upper)
```
...
lower_99 <- samp_mean - z_99 * samp_sd/sqrt(n)
upper_99 <- samp_mean + z_99 * samp_sd/sqrt(n)

plot_ci(lower_99, upper_99, true_mean)
```
...
```{r}
ci_wihout_pop_mean <- sum(true_mean < lower | true_mean > upper)
ci_wihout_pop_mean
```
...
df <- n -1
t_star <- qt(.975, df)
lower_t <- samp_mean - t_star * samp_sd/sqrt(n)
upper_t <- samp_mean + t_star * samp_sd/sqrt(n)
plot_ci(lower_t, upper_t, true_mean)

t_ci_wihout_pop_mean <- sum(true_mean < lower_t | true_mean > upper_t)
t_ci_wihout_pop_mean < ci_wihout_pop_mean
```
